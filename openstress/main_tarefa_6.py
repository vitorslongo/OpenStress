...
class Main:
    ...
    def get_constitutive_matrix(self, EPT=False, EPD=False):
        ...
        if EPT:
            ...
            coef = E*(1-self.nu)/((1+self.nu)*(1-2*self.nu))
            constitutive_matrix = coef*np.array([[1, self.nu/(1-self.nu), 0],
                                                 [self.nu/(1-self.nu), 1, 0],
                                                 [0, 0, (1-2*self.nu)/(2*(1-self.nu))]])


        else:
            raise Exception("Please select a case, EPT or EPD.")
    ...
    def apply_nodal_forces(self, loads=list):
        ...
        loads: [nodes, fx, fy]
        """
        if not self.check_bc_format(loads):
            raise TypeError("Boundary condition must me a list of lists")
    ...
    def apply_dirichlet(self, dirichlet):
            """
            dirichlet: [node_idx, value, x, y]
            """
            if not self.check_bc_format(dirichlet):
                raise TypeError("Boundary condition must me a list of lists")
    ...
    def von_mises(self, sx, sy, txy):
        return np.sqrt(sx*sx - sx*sy + sy*sy + 3*(txy**2))
    ...
    def print_results(self):
        # Control nodes:
        intern_radius_right_node = 0
        outer_radius_right_node = 1

        # • Ure = Deslocamento radial externo;
        # • Sri = Tens˜ao radial interna. Comparar com valor exato;
        # • Sre = Tens˜ao radial externa. Comparar com valor exato;
        # • Sti = Tens ˜ao tangencial (circunferencial) no raio interno; Comparar com solu c¸˜ao de tubo de parede
        # fina;
        # • Ste = Tens ˜ao tangencial (circunferencial) no raio externo; Comparar com solu c¸˜ao de tubo de parede
        # fina;
        # • Sa = Tens˜ao axial;
        # • Svmi = Tens˜ao de von Mises no raio interno;
        # • Svmi = Tens˜ao de von Mises no raio externo;  
        
        Uri = self.U[intern_radius_right_node][0]
        Ure = self.U[outer_radius_right_node][0]
        print("Deslocamento radial interno:", round(Uri*1e6, 3), "µm")
        print("Deslocamento radial externo:", round(Ure*1e6, 3), "µm\n")

        Sri = self.sigma_nodal[intern_radius_right_node][0]
        Sre = self.sigma_nodal[outer_radius_right_node][0]
        print("Tensão radial interna:", round(Sri*1e-6, 3), "MPa")
        print("Tensão radial externa:", round(Sre*1e-6, 3), "MPa\n")

        Sti = self.sigma_nodal[intern_radius_right_node][1]
        Ste = self.sigma_nodal[outer_radius_right_node][1]
        print("Tensão tangencial interna:", round(Sti/1e6, 3), "MPa")
        print("Tensão tangencial externa:", round(Ste/1e6, 3), "MPa\n")

        Svmi = self.vm[intern_radius_right_node]
